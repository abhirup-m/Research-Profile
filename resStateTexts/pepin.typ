...
#balance(columns( 2,[
I am a quantum condensed matter theorist focusing on characterising phases of strongly correlated *quantum matter* - novel states that appear at zero temperature and display emergent exotic properties, often driven by strong interactions between the constituent particles. Such phases are interesting because they cannot be organised according to the paradigm of spontaneous symmetry breaking and local order parameters. My work presently focuses on understanding various features of Mott metal-insulator transition from the perspective of *Kondo breakdown*. Approaching the physics of the Mott MIT from this "local perspective" offers valuable insight into topological and momentum-selective transitions as well as patterns of entanglement near quantum criticality. This connects closely with your work on correlated systems (such as variants of the Hubbard model) at IPhT.

In particular, I have been following your recent work on *layer-selective* transitions, superconductivity and topological features of various Hubbard models. Given my experience in quantum impurity models and entanglement calculations, and the versatility of the *composite-operator method* routinely employed by you, I envisage several avenues for our collaboration. These include (i) entanglement calculations in a layer-selective Hubbard model, (ii) using the COM as a DMFT solver to reduce computational cost, and (iii) apply the COM to magic-angle TBG and compare against an exact solution from a Hatsugai-Kohmoto variant. These ideas are elaborated in more detail below.
]))
...
*Entanglement calculations in layer-selective Hubbard model*. As a follow up to your work on the bi-laye Hubbard model (PRB 110, 235142 (2024)), one can use the COM to calculate various entanglement measures and track various phase boundaries. The measures can also be used to determine the presence of long-range correlations, particularly near critical points.

*COM for MATBG*. Given the versatility of the composite-operator method, we can apply it to an interacting model of MATBG, and study the various phases therein. The method already allows for the calculation of topologcal invariants (PRB 112, 085105 (2025)). In addition, we can compare these results to those obtained from the exactly-solvable Hatsugai-Kohmoto model. Instabilities towards non-BCS superconductivity can also be explored and compared against experiments.

*Using COM as an impurity solver in DMFT*. The most computationally intensive step of the dynamical mean-field theory algorithm is the impurity solver, where one computes the local Greens function of the impurity model. There might be advantages of doing this using the COM, such as (i) less intensive than NRG, (ii) no sign problem unlike QMC, (iii) more accurate than IPT.
