...
#balance(columns( 2,[
I am a quantum condensed matter theorist focusing on characterising phases of strongly correlated *quantum matter* - novel states that appear at zero temperature and display emergent exotic properties, often driven by strong interactions between the constituent particles. Such phases are interesting because they cannot be organised according to the paradigm of spontaneous symmetry breaking and local order parameters. My work presently focuses on understanding various features of Mott metal-insulator transition from the perspective of *Kondo breakdown*. Approaching the physics of the Mott MIT from this "local perspective" and incorporating lattice geometry offers valuable insight into Fermi surface topology, pseudogap physics and correlation-driven transitions. This connects closely with your work on correlated systems (such as variants of the Hubbard model) at MPI-FKF.

In particular, I have been following your recent works on the three-band Emery model and on the real space fluctuation diagnostics approach. Given my experience in correlated electronic models and entanglement calculations, I envisage several avenues for our collaboration. These include (i) extending arXiv:2412.14951 by studying the Emery model at the exactly solvable *Hatsugai-Kohmoto* limit to get a handle on non-Fermi liquid physics/PG/superconductivity, and (ii) using the real space fluctuation diagnostics approach to study *entanglement patterns* as a probe for the Mott MIT. These ideas are elaborated in more detail below.
]))
...
#balance(
  columns(2, [
*Analysing the exactly-solvable Hatsugai-Kohmoto limit of the three-band Emery model*. The Hatsugai-Kohmoto model $H_"KM"$ is an exactly solvable model of correlated electrons, displaying non-Fermi liquid excitations, fermi arcs and unconventional superconductivity. It might be interestin to replace the Hubbard interaction within the Emery model with the HK interaction and study the resulting phases thoroughly by making use of the analytic solvability. Non-solvable interaction forms can also be added perturbatively to investigate deviations.

*Using the real space fluctuation diagnostics approach to study entanglement patterns as a probe for the Mott MIT*. This combines the work of the arXiv:2311.09023 (DMFT-based calculation of single-particle spectra in the Emery model) and PRR 7, 033073 (introduction of the real space fluctuation diagnostics approach). The real space fluctuation diagnostics approach (RSFDA) is already able to isolate the scattering processes that lead to a pole in the one-particle self-energy (the Mott gap). It should be interesting to try and extend the DMFT calculations on the DMFT model to calculate real-space entanglement (using subsystem correlation functions) and correlate entanglement signatures with the self-energy signatures obtained using the RSFDA.
